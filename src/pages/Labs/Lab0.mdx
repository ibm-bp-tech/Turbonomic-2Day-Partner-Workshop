...
Kubeturbo can be installed on Kubernetes version 1.11 or higher or OpenShift release 3.11 and higher on any x86_64, Power or LinuxONE architecture. The other requirements are
...
- Now run this command to install kubeturbo. **Make sure to change the password for restAPIConfig.opsManagerPassword and set it to the one provided in the Credentials file. Be sure to remove any quotation marks ''. You can use a text editor for this. To access the text editor navigate to "Activities" from your bastion vm and open click on the nine dots to open all apps. then select text editor.**
